...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      >
        <div className="flex items-center gap-3">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="left" className="w-[280px] p-0 pt-16">
              <div className="h-full flex flex-col">
                ...
                <Button variant="ghost" size="icon" className="absolute top-3 right-3" onClick={() => setOpen(false)}>
                  <X className="h-5 w-5" />
                </Button>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-semibold hidden md:block">FuelTrack</span>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? "text-primary font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted",
                )}
              >
      ...
      {!isAddLogPage && (
        <Link href="/logs/add" className="fixed bottom-24 right-6 md:hidden flex items-center justify-center z-40">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-background border-t z-40 flex items-center justify-around px-2">
        {navItems.slice(0, 4).map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} className="flex flex-col items-center justify-center py-1 px-3">
              <motion.div
                animate={{
                  y: isActive ? -2 : 0,
              ...
              {isActive && (
                <motion.div layoutId="mobile-nav-indicator" className="h-1 w-4 rounded-full bg-primary mt-1" />
              )}
            </Link>
          )
        })}
